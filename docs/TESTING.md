...
After the CR-016 correction, three repeated Lighthouse runs against the same rebuilt production server produced a median performance score of 0.91, FCP 1.65s, LCP 3.38s, CLS 0.001 and 494 KiB transfer. PerformanceObserver identified the homepage hero `H1` as LCP; the editorial image is below the fold, remains lazy, and the browser selects the 640w AVIF candidate for its 520px desktop slot. The media delivery audit no longer reports image waste. The LCP target remains an open performance follow-up; no field or p75 claim is made.
