...
The four continuously maintained dashboards now have one canonical location each: [Human Decisions](docs/HUMAN_DECISIONS.md), [Risk Register](docs/RISK_REGISTER.md), [Technical Register](docs/TECHNICAL_REGISTER.md), [Lean Register](docs/LEAN_REGISTER.md). Material post-baseline delivery changes are tracked in the fifth [Change Register](CHANGE_REGISTER.md). §15.5 retains governance schemas and historical initial records only; do not update a competing dashboard here. Current ownership/contracts: [Architecture](docs/ARCHITECTURE.md). Production Q05/Q06/Q07/Q10 remain open and do not block implementation.
...
The specification selects an Astro-based direction. Delivery is one modular TypeScript application: prerendered public content, progressively enhanced forms and local search, server-rendered private administration, a first-party server, and SQLite on persistent Kora-controlled storage. All runtime assets and functionality remain first-party. Implementation is active in the GitHub `main` branch; production publication remains gated by approved content, privacy, hosting and operational decisions.

The product is technically feasible within these boundaries. Final content/mapping ownership, privacy/retention approval and hosting/operations are production gates, not prerequisites for architecture, UX scaffolding, schemas, tests or implementation. Routine technical choices are recorded engineering decisions. The user resolved Q03: partner forms require contact name, email and a meaningful capability/partnership summary; organization is optional. Reverse/reopen status transitions are excluded rather than assumed. Delivery follows the Orchestrator/specialist ownership, contract and evidence gates in §15; this is implementation governance, not additional product scope.
...
This inventory records the original Phase 1 workspace, including hidden entries; it is historical and does not describe the current implementation tree. No unrelated directories or credential stores were inspected. No external sources were consulted: technology names here derive from P §28. Embedded packaging/style metadata is not an additional project specification; the document text was decoded without extracting or modifying the authoritative files.
...
| P §24 | Custom geometric K/path/convergence SVG master; horizontal/symbol/dark/light logo variants; one-color/favicon viability; accessible brand name. Approved Deep Black #070909 / Premium Gold #D4AF37 / Off-White #F2F2F2 system with graphite, mist and semantic error/warning/success/focus tokens; verify combinations and never use gold as failing small text. Local self-hosted Manrope supporting typeface in limited weights; roughly 16px mobile body and 45–80-character reading lines | Brand/design review and asset/license/contrast checks; exact proposed colors stay in P §24.4 |
...
| P §26 | Free-standard editorial media discovered, reviewed, downloaded, optimized and served locally with provenance; neutral representation, responsive derivatives, alt text, metadata/disclosure review and no provider runtime calls | `content/media/media-manifest.json`, local AVIF/WebP/JPEG assets, `EditorialImage.astro`, `verify:media`, visual/rights/accessibility/performance review |
...
| Icons/brand/media | Custom SVG logo, local SVG icons, local licensed Manrope font files and optimized editorial media | Required first-party assets and brand deliverables; generated raster derivatives are permitted for photography, never for the canonical logo |
...
| Hosting | Approved baseline: Cloudflare Free edge → Kora-controlled DigitalOcean NYC3 Linux Droplet → Caddy → systemd → Astro/Node 24 → private SQLite; containerized deployment remains optional | Portable self-hosting, small operational footprint. Q07 topology/domain/operator/recovery decision is closed; provisioning, DNS/TLS, firewall, permissions, encrypted backup/key custody, staging, hardening and restore rehearsal remain execution gates |
...
| **Q07** | **CLOSED HUMAN DECISION; EXECUTION GATES REMAIN** | Approved Cloudflare Free edge, Kora-controlled DigitalOcean NYC3 Linux Droplet in NYC3, canonical `https://korasb.com`, Caddy → systemd → Astro/Node 24 → private SQLite, `support@korasb.com` as operational and backup/recovery contact, encrypted off-host backups with separate key control, and mandatory restore rehearsal. Provisioning, DNS/TLS, firewall/private listener, filesystem permissions, backup/key implementation, staging, hardening and cutover still block production deployment. |
...
**Q01 only:** explicitly approve this revised plan for implementation. Until then, stop. Q03 is answered, absorbed into PD-001 (§5.1.1), and closed. No other current human decision blocks baseline implementation. Q05 remains a production content gate; Q06/Q10 are absorbed; Q07 human topology is closed while its execution gates remain pre-production requirements. Unrelated approved work must continue when one is open. Out-of-scope extensions are not pending tasks or approval requests.
...
The original Phase 1 verification was limited to document reading, inventory, requirement-ID coverage and source fingerprint comparison. Current implementation evidence is maintained in the Lean, Technical, Risk and Change registers; the product remains in release-readiness work and is not claimed production-ready while its documented gates remain open.
