...
export function listInquiries(filters: Record<string,string|undefined> = {}) { const where:string[]=[]; const args:any[]=[]; for(const key of ['type','routing_queue','status'] as const){if(filters[key]){where.push(`${key}=?`);args.push(filters[key])}} const sql=`SELECT * FROM inquiries ${where.length?'WHERE '+where.join(' AND '):''} ORDER BY created_at DESC`; return getDb().prepare(sql).all(...args) as any[]; }
